...
class GaussianExtractor(object):
    ...
    @torch.no_grad()
    def reconstruction(self, viewpoint_stack, relight=False, rescale=False, traj=False):
        """
        ...
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack),
            ...
            self.specularmaps.append(render_pkg['rend_specular_color'].cpu())
            ...
            if not relight:
                self.residualmaps.append(render_pkg['rend_specular_residual'].cpu())
                ...
                self.diffusemaps.append(render_pkg['rend_diffuse'].cpu())
                self.tintmaps.append(render_pkg['rend_tint'].cpu())

        if relight and rescale:
            # adjust brightness for relighted scene based on gt
            rescale_brightness_multichannel = True
            ratios = []
            for i in range(len(self.rgbmaps)):
                mask = self.alphamaps[i].squeeze() > 0.5
                if rescale_brightness_multichannel:
                    gt_mean = self.gts[i][:3, mask].mean(axis=1)
                    render_mean = self.rgbmaps[i][:3, mask].mean(axis=1)
                else:
                    gt_mean = self.gts[i][:3, mask].mean()
                    render_mean = self.rgbmaps[i][:3, mask].mean()
                ratios.append(gt_mean / torch.max(render_mean, torch.tensor(1e-6)))

            if rescale_brightness_multichannel:
                factor = torch.vstack(ratios).mean(axis=0)[:, None, None]
            else:
                factor = torch.mean(torch.tensor(ratios))

            print("Relighting brightness scaling factor: ", factor.squeeze().tolist())
            for i in range(len(self.rgbmaps)):
                self.rgbmaps[i] = factor * self.rgbmaps[i] * self.alphamaps[i] + (1 - self.alphamaps[i])
    ...
    @torch.no_grad()
    def export_image(self, path, relight=False, skip_misc=False, traj=False):
        if not traj:
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images", total=len(self.viewpoint_stack)):
            if 0 < self.num_render <= idx:
                break

            save_img_u8(self.rgbmaps[idx].permute(1,2,0).cpu().numpy(), os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
            save_img_u8(self.specularmaps[idx].permute(1,2,0).cpu().numpy(), os.path.join(vis_path, 'specular_{0:05d}'.format(idx) + ".png"))
            if not traj:
            ...
            if not skip_misc:
                save_img_u8(self.residualmaps[idx].permute(1,2,0).cpu().numpy(), os.path.join(vis_path, 'residual_{0:05d}'.format(idx) + ".png"))
                save_img_f32(self.depthmaps[idx][0].cpu().numpy(), os.path.join(vis_path, 'depth_{0:05d}'.format(idx) + ".tiff"))
                save_img_u8(self.normals[idx].permute(1,2,0).cpu().numpy() * 0.5 + 0.5, os.path.join(vis_path, 'normal_{0:05d}'.format(idx) + ".png"))
                save_img_u8(self.depth_normals[idx].permute(1,2,0).cpu().numpy() * 0.5 + 0.5, os.path.join(vis_path, 'depth_normal_{0:05d}'.format(idx) + ".png"))
                save_img_u8(self.roughnessmaps[idx].permute(1,2,0).squeeze(-1).cpu().numpy(), os.path.join(vis_path, 'roughness_{0:05d}'.format(idx) + ".png"))
                save_img_u8(self.diffusemaps[idx].permute(1,2,0).cpu().numpy(), os.path.join(vis_path, 'diffuse_{0:05d}'.format(idx) + ".png"))
                save_img_u8(self.tintmaps[idx].permute(1,2,0).cpu().numpy(), os.path.join(vis_path, 'tint_{0:05d}'.format(idx) + ".png"))